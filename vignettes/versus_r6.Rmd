...
Q7 is expected to be compared with R6. The main difference 

Technically, Q7 is expected to suffer from slight performance deficiencies when compared to R6. 
...
- Hereditary: I made my spaceship a special kind of spaceship that has a plasma cannon, then further made it special with an expanded cargo area.
...
In Q7, new code is simply appened to the old, meaning everything will be executed linearly from the beginning to the end. This make it inefficient when you replace something costly to make, like reading in a large amount of data or performing a lengthy calculation. In this case, it's best to make a new type from scratch, or define a common prototype.

Q7 type constructors need not (and cannot) be locked, as they are functions.
...
print.prettyCountingQueue <- function(x){
  cat("<PrettyCountingQueue> of ", x$get_total(), " elements\n", sep = "")
}

pq <- prettyCountingQueue(1, 2, "foobar")
pq
```
Q7 objects do not contain a bound `print()` function. A free `print()` S3 method must be accessible by the calling scope. It is also necessary to extend the prototype in order to give the new type a S3 class for correct method dispatch. 
...
Q7 will not wrap your finalizer function, so you must provide include an argument (could be any name) to represent the object itself.
